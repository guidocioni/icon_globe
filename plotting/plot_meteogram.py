...
def main():
    ...
    args = dict(dset=dset, hsurf=hsurf, time=time, lons=lons, lats=lats)
...
def plot(cities, **args):
    for city in cities:
        ...
        cs2 = ax0.contour(args['time'], plevs, rh.T,
        ...
        ts1 = ax1.plot(args['time'], td2m, label='2m $T_d$', color='darkcyan', linestyle='dashed')
        ...
        for dt, weather_icon, dewp in zip(args['time'], weather_icons, dset_city['2t']):
        ...
        ts = ax2.plot(args['time'], vmax_10m, label='Gusts', color='lightcoral')
        ...
        ts = ax3.plot(args['time'], rain_acc, label='Rain (acc.)', color='dodgerblue', linewidth=0.1)
        ts1 = ax3.plot(args['time'], snow_acc, label='Snow (acc.)', color='orchid', linewidth=0.1)
